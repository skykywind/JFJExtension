...
public class CacheHelper {
    ...
    private var cacheQueue = DispatchQueue.init(label: "com.CacheHelper.cacheQueue")
    
    public static var shared = CacheHelper(cacheName: Bundle.main.infoDictionary?["TargetName"] as? String ?? "MyAppCache")
    ...
    /// This initializer method will use ~/Library/Caches/CacheHelper/targetName to save data
    ///
    /// - Parameter cacheName: Name of the cache (by default it is 'TargetName')
    private init(cacheName: String) {
        let dir = PathHelper.cachePath + "/CacheHelper/" + cacheName
